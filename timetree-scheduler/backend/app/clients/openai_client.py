"""
OpenAI ChatGPT client for natural language processing and event parsing.

Handles communication with OpenAI's ChatGPT API for parsing Korean text into structured event data.
"""
...
class OpenAIParseError(OpenAIAPIError):
    """Exception raised when OpenAI response cannot be parsed."""
    
    def __init__(self, response_text: str, parse_error: str):
        self.response_text = response_text
        self.parse_error = parse_error
        super().__init__(f"Failed to parse OpenAI response: {parse_error}", "parse_error")


class OpenAIClient:
    """
    OpenAI ChatGPT client for natural language processing.
    ...
    def _load_system_prompt(self) -> str:
        ...
        try:
            ...
            raise OpenAIAPIError(f"Failed to load system prompt: {str(e)}", "prompt_load_error")
    ...
    async def parse_natural_language(
        ...
        Raises:
            OpenAIAPIError: On API or parsing errors
        """
        try:
            # Prepare context
...
"""
            ...
            try:
                ...
                if json_start >= 0 and json_end > json_start:
                    try:
                        ...
                        raise OpenAIParseError(response_text, str(e))
                else:
                    raise OpenAIParseError(response_text, str(e))
            ...
            try:
                ...
                logger.error("Failed to validate parsed data",
                           ...
                           error=str(e))
                raise OpenAIAPIError(f"Invalid parsed data structure: {str(e)}", "validation_error")
            ...
            raise OpenAIAPIError(f"API request failed: {str(e)}", "api_error")
    ...
    async def parse_with_retry(
        ...
        except (OpenAIParseError, OpenAIAPIError) as e:
...
"""
            ...
            response = await self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are a calendar event validation assistant. Respond only in JSON format."
                    },
                    {
                        "role": "user",
...
"""
            ...
            response = await self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are a calendar event suggestion assistant. Respond only in JSON format."
                    },
                    {
                        "role": "user",
