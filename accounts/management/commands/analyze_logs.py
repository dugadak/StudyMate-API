    ...
    def handle(self, *args, **options):
        ...
        if options["export"]:
            with open(options["export"], "w") as f:
                f.write(output)
            self.stdout.write(self.style.SUCCESS(f"Report exported to {options['export']}"))
        else:
...
class LogAnalyzer:
    ...
    def analyze(self) -> Dict[str, Any]:
        ...
        report = {
            "period": {"start": self.start_date.isoformat(), "end": self.end_date.isoformat(), "days": self.days},
            "summary": {"total_entries": 0, "files_analyzed": []},
        }
