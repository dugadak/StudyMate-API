...
class Command(BaseCommand):
    ...
    def analyze_all(self):
        ...
        for app_name in target_apps:
            try:
                ...
                self.stdout.write(
                    self.style.ERROR(f'Error analyzing {app_name}: {str(e)}')
                )
        ...
        for query_name, query_func in queries_to_test:
            try:
                ...
                if duration > 100:  # Slow query threshold
                    self.stdout.write(
                        self.style.WARNING(f'    ⚠️  {query_name}: {duration:.2f}ms (SLOW)')
                    )
                elif duration > 50:
                    self.stdout.write(
                        self.style.HTTP_INFO(f'    🔶 {query_name}: {duration:.2f}ms')
                    )
                else:
    ...
    def analyze_slow_queries(self):
        ...
        try:
            ...
            self.stdout.write(
                self.style.ERROR(f'    Error analyzing slow queries: {str(e)}')
            )
    ...
    def analyze_table_sizes(self):
        ...
        try:
            ...
            self.stdout.write(
                self.style.ERROR(f'    Error analyzing table sizes: {str(e)}')
            )
    ...
    def analyze_index_usage(self):
        ...
        try:
            ...
            self.stdout.write(
                self.style.ERROR(f'    Error analyzing index usage: {str(e)}')
            )
