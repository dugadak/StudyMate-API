...
from studymate_api.cache import smart_cache, cache_warmer, cache_invalidator, CacheMonitor, StudyMateCache
import json
...
class Command(BaseCommand):
    ...
    def show_cache_info(self):
        ...
        self.stdout.write(self.style.SUCCESS("=== StudyMate Cache Information ===\n"))
    ...
    def warm_caches(self, cache_type: str = None):
        """Warm up caches"""
        self.stdout.write(self.style.HTTP_INFO("🔥 Warming up caches...\n"))

        if cache_type:
            # Warm specific cache type
            try:
                cache_warmer.warm_cache(cache_type)
                self.stdout.write(self.style.SUCCESS(f"✅ Cache warming completed for {cache_type}"))
            except Exception as e:
                self.stdout.write(self.style.ERROR(f"❌ Cache warming failed for {cache_type}: {str(e)}"))
        else:
            # Warm all caches
            try:
                cache_warmer.warm_all_caches()
                self.stdout.write(self.style.SUCCESS("✅ All cache warming completed"))
            except Exception as e:
    ...
    def clear_caches(self, cache_type: str = None, pattern: str = None):
        """Clear caches"""
        self.stdout.write(self.style.WARNING("🗑️  Clearing caches...\n"))

        if cache_type and pattern:
            # Clear specific pattern
            deleted_count = smart_cache.delete_pattern(cache_type, pattern)
            self.stdout.write(self.style.SUCCESS(f"✅ Deleted {deleted_count} cache entries for {cache_type}:{pattern}"))
        elif cache_type:
            # Clear all entries for cache type
            deleted_count = smart_cache.delete_pattern(cache_type)
            self.stdout.write(self.style.SUCCESS(f"✅ Deleted {deleted_count} cache entries for {cache_type}"))
        else:
            # Clear entire cache
            try:
                cache.clear()
                ...
                self.stdout.write(self.style.ERROR(f"❌ Cache clearing failed: {str(e)}"))

    def show_statistics(self):
        """Show detailed cache statistics"""
        self.stdout.write(self.style.SUCCESS("=== Cache Performance Statistics ===\n"))
    ...
    def test_cache_performance(self):
        ...
        self.stdout.write(self.style.HTTP_INFO("🧪 Testing cache performance...\n"))
        ...
        if retrieved_data == test_data:
            self.stdout.write(self.style.SUCCESS("✅ Data integrity test passed"))
        else:
            self.stdout.write(self.style.ERROR("❌ Data integrity test failed"))
        ...
        if set_time > 10:
            self.stdout.write(self.style.WARNING("   ⚠️  SET operations are slow (>10ms)"))

        if get_time > 5:
            self.stdout.write(self.style.WARNING("   ⚠️  GET operations are slow (>5ms)"))

        if bulk_get_time / 100 > 2:
            self.stdout.write(self.style.WARNING("   ⚠️  Consider implementing batch operations"))

        hit_rate = smart_cache.get_stats()["hit_rate"]
        if hit_rate < 80:
            self.stdout.write(self.style.WARNING(f"   ⚠️  Cache hit rate is low ({hit_rate}%)"))
