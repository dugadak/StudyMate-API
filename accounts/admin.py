...
from .models import User, UserProfile, EmailVerificationToken, PasswordResetToken, LoginHistory
...
class LoginHistoryInline(admin.TabularInline):
    ...
    max_num = 10
    can_delete = False
    readonly_fields = ("ip_address", "user_agent", "success", "failure_reason", "location", "created_at")
...
class UserAdmin(BaseUserAdmin):
    ...
    fieldsets = (
        ...
        ("활동 기록", {"fields": ("last_login", "last_activity", "date_joined", "created_at", "updated_at")}),
        (
            "보안 요약",
            {
    ...
    actions = ["unlock_accounts", "lock_accounts", "verify_emails", "force_password_change", "export_user_data"]
    ...
    def account_status(self, obj: User) -> str:
        ...
        if not obj.is_active:
            ...
            return format_html('<span style="color: orange;">주의 ({}회 실패)</span>', obj.failed_login_attempts)
        else:
    ...
    def unlock_accounts(self, request, queryset):
        ...
        self.message_user(request, f"{count}개의 계정이 잠금 해제되었습니다.")
    ...
    def lock_accounts(self, request, queryset):
        ...
        self.message_user(request, f"{count}개의 계정이 1시간 동안 잠금되었습니다.")
    ...
    def verify_emails(self, request, queryset):
        ...
        self.message_user(request, f"{count}개의 이메일이 인증되었습니다.")
    ...
    def force_password_change(self, request, queryset):
        ...
        count = queryset.update(last_password_change=timezone.now() - timezone.timedelta(days=91))
        self.message_user(request, f"{count}명의 사용자에게 비밀번호 변경이 필요하도록 설정되었습니다.")
...
class UserProfileAdmin(admin.ModelAdmin):
    ...
    list_display = ["user_email", "name", "country", "language", "theme", "study_stats", "created_at"]
    ...
    fieldsets = (
        ("기본 정보", {"fields": ("user", "name", "bio", "avatar")}),
        ("연락처", {"fields": ("phone_number", "country", "timezone")}),
        ("설정", {"fields": ("language", "theme")}),
        (
            "학습 통계",
            {
...
class LoginHistoryAdmin(admin.ModelAdmin):
    ...
    list_display = ["user_email", "ip_address", "success_status", "failure_reason", "location", "created_at"]
